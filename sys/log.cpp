...
// added by verminniu 2008-3-25
// 访问日志
class CLog *g_pAccLog = NULL;

class CLog *g_pStatsLog = NULL;
// end added
...
CLog::shilft_files()
{
    ...
    sprintf(sLogFileName, "%s.log.%d", _log_base_path, _max_log_num - 1);
    ...
    for (i = _max_log_num - 2; i >= 0; i--) {
        if (i == 0) {
            ...
            sprintf(sLogFileName, "%s.log.%d", _log_base_path, i);
        }

        if (access(sLogFileName, F_OK) == 0) {
            sprintf(sNewLogFileName, "%s.log.%d", _log_base_path, i + 1);
